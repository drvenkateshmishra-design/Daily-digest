...
Newspaper-styled: serif (Times) typography throughout, a proper masthead with
double rules, and each news item rendered as headline + excerpt + byline,
like an actual paper rather than a bare link list.
"""
...
def _styles():
    ...
    # ---- Masthead ----
    ss.add(ParagraphStyle("Masthead", parent=ss["Title"], fontName="Times-Bold",
                           textColor=NAVY, fontSize=30, leading=34,
                           alignment=TA_CENTER, spaceAfter=2))
    ss.add(ParagraphStyle("Tagline", parent=ss["Normal"], fontName="Times-Italic",
                           textColor=GREY, fontSize=10, alignment=TA_CENTER,
                           spaceAfter=2))
    ss.add(ParagraphStyle("DateLine", parent=ss["Normal"], fontName="Times-Roman",
                           textColor=INK, fontSize=9.5, alignment=TA_CENTER,
...
def build_pdf(news, journals, output_path):
    ...
    # ---- MASTHEAD ----
    story.append(HRFlowable(width="100%", thickness=2.2, color=NAVY, spaceAfter=4))
    story.append(Paragraph(config.PDF_TITLE.upper(), ss["Masthead"]))
    story.append(Paragraph("News &amp; Journal Digest", ss["Tagline"]))
    story.append(HRFlowable(width="100%", thickness=0.6, color=NAVY, spaceBefore=3, spaceAfter=3))
    story.append(Paragraph(today_str, ss["DateLine"]))
    story.append(HRFlowable(width="100%", thickness=2.2, color=NAVY, spaceAfter=2))
    ...
    for section, items in news.items():
        ...
        for i, it in enumerate(items):
            ...
            story.append(Paragraph(headline, ss["Headline"]))
            if it.get("excerpt"):
                story.append(Paragraph(it["excerpt"], ss["Excerpt"]))
            story.append(Paragraph(_byline(it), ss["Byline"]))
            if i < len(items) - 1:
    ...
    for section, items in journals.items():
        ...
        for i, it in enumerate(items):
            ...
            story.append(Paragraph(meta, ss["Byline"]))
            if i < len(items) - 1:
                story.append(HRFlowable(width="35%", thickness=0.4, color=LIGHT_RULE,
                                         spaceBefore=4, spaceAfter=2, hAlign="LEFT"))
